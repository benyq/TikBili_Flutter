...
class SliderLayer extends ConsumerStatefulWidget {
  ...
  const SliderLayer(
      {super.key,
        required this.videoItem,this.onTap});
...
class _SliderLayerState extends ConsumerState<SliderLayer> {
  ...
  Widget build(BuildContext context) {
    final sliding = ref.watch(videoItemViewModelProvider(widget.videoItem).select((v)=>v.layerState.isSliding));
    final slideValue = ref.watch(videoItemViewModelProvider(widget.videoItem).select((v)=>v.layerState.slideValue));
    //TODO 直接拿position会导致页面不停地刷新,要用timer
    final position = ref.watch(videoItemViewModelProvider(widget.videoItem).select((v)=>v.playerState.playPosition));
    final duration = ref.watch(videoItemViewModelProvider(widget.videoItem).select((v)=>v.videoDetail.duration));
    final radio = position / duration;
    return SizedBox(
    ...
    Stack(
      children: [
        Positioned(child: sliding ? SizedBox(
          width: double.maxFinite,
          height: 200.w,
            ...
            child: SizedBox(
              ...
              Row(
                children: [
                  Expanded(
                      child: Slider(
                        value: sliding ? slideValue : radio,
                        onChanged: (v) {
